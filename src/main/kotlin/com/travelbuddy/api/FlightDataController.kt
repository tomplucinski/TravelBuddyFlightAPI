...
class FlightDataController {
    ...
    fun getQuotes(@RequestParam origin: String, destination: String, date: String): ResponseEntity<String>? {

        val client = HttpClient.newBuilder().build()
        val request = HttpRequest.newBuilder()
                .uri(URI.create(String.format("%s%s%s/%s/%s", Constants.QUOTES, "US/USD/en-US/", origin, destination, date)))
                .setHeader("x-rapidapi-host", "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com")
                .setHeader("x-rapidapi-key", appProperties.rapidApiKey)
                .setHeader("Content-Type", "application/json")
                .build()
